...
{
    [super loadView];
    [self setTitleLabel:NSLocalizedString(@"AAMFeedbackTitle", nil)];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if (section == 0) {
        label.text = NSLocalizedString(@"AAMFeedbackTableHeaderTopics", nil);
    }
    label.text = NSLocalizedString(@"AAMFeedbackTableHeaderBasicInfo", nil);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(indexPath.section==0 && indexPath.row==0){
        ...
        AAMFeedbackTopicsViewController *vc = [[[AAMFeedbackTopicsViewController alloc]initWithStyle:UITableViewStylePlain]autorelease];
        vc.delegate = self;
